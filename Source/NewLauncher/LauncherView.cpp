    ...
    const LauncherViewEvents& events) noexcept
{
    ...
    if (opened)
    {
        ...
        else
        {
            ...
            // Primary Action: Epic Game Start Button with glowing sapphire crystals (Transitions to Login Form on click)
            ImGui::SetCursorPos(ImVec2(winSize.x - 148, bottomCardY + 28));
            const bool gameLocked = !state.isGameEnabled;
            if (RenderEpicGameStartButton(
                    "##play_btn",
                    ImVec2(122, 62),
                    gameLocked))
            {
...
static IDirect3DTexture9* LoadD3D9TextureFromFile(
    ...
    ImVec2& outUvMax)
{
    ...
    if (SUCCEEDED(lockHr))
    {
        ...
        outUvMax = ImVec2(
            static_cast<float>(width) / static_cast<float>(texW),
            static_cast<float>(height) / static_cast<float>(texH));
...
void LauncherView::LoadButtonTextures(IDirect3DDevice9* device, const std::wstring& exeDir) noexcept
{
    if (!device) return;
    if (btnStartNormalTex_) return;

    std::vector<std::filesystem::path> pathsNormal;
    std::vector<std::filesystem::path> pathsHover;
    std::vector<std::filesystem::path> pathsPress;
    std::vector<std::filesystem::path> pathsDisabled;

    if (!exeDir.empty())
    {
        const std::filesystem::path dir(exeDir);
        pathsNormal.push_back(dir / L"LauncherData" / L"assets" / L"buttons" / L"btn_start_normal.png");
        pathsNormal.push_back(dir / L"assets" / L"buttons" / L"btn_start_normal.png");
        pathsNormal.push_back(dir / L"btn_start_normal.png");

        pathsHover.push_back(dir / L"LauncherData" / L"assets" / L"buttons" / L"btn_start_hover.png");
        pathsHover.push_back(dir / L"assets" / L"buttons" / L"btn_start_hover.png");
        pathsHover.push_back(dir / L"btn_start_hover.png");

        pathsPress.push_back(dir / L"LauncherData" / L"assets" / L"buttons" / L"btn_start_press.png");
        pathsPress.push_back(dir / L"assets" / L"buttons" / L"btn_start_press.png");
        pathsPress.push_back(dir / L"btn_start_press.png");

        pathsDisabled.push_back(dir / L"LauncherData" / L"assets" / L"buttons" / L"btn_start_disabled.png");
        pathsDisabled.push_back(dir / L"assets" / L"buttons" / L"btn_start_disabled.png");
        pathsDisabled.push_back(dir / L"btn_start_disabled.png");
    }
    pathsNormal.push_back(L"LauncherData/assets/buttons/btn_start_normal.png");
    pathsNormal.push_back(L"assets/buttons/btn_start_normal.png");
    pathsNormal.push_back(L"btn_start_normal.png");

    pathsHover.push_back(L"LauncherData/assets/buttons/btn_start_hover.png");
    pathsHover.push_back(L"assets/buttons/btn_start_hover.png");
    pathsHover.push_back(L"btn_start_hover.png");

    pathsPress.push_back(L"LauncherData/assets/buttons/btn_start_press.png");
    pathsPress.push_back(L"assets/buttons/btn_start_press.png");
    pathsPress.push_back(L"btn_start_press.png");

    pathsDisabled.push_back(L"LauncherData/assets/buttons/btn_start_disabled.png");
    pathsDisabled.push_back(L"assets/buttons/btn_start_disabled.png");
    pathsDisabled.push_back(L"btn_start_disabled.png");

    btnStartNormalTex_ = LoadD3D9TextureFromFile(device, pathsNormal, btnStartUvMin_, btnStartUvMax_);
    btnStartHoverTex_ = LoadD3D9TextureFromFile(device, pathsHover, btnStartUvMin_, btnStartUvMax_);
    btnStartPressTex_ = LoadD3D9TextureFromFile(device, pathsPress, btnStartUvMin_, btnStartUvMax_);
    btnStartDisabledTex_ = LoadD3D9TextureFromFile(device, pathsDisabled, btnStartUvMin_, btnStartUvMax_);

    if (btnStartNormalTex_)
    {
        Logger::Log("Epic Game Start sapphire button textures loaded successfully.");
    }
}

void LauncherView::UnloadButtonTextures() noexcept
{
    if (btnStartNormalTex_)
    {
        btnStartNormalTex_->Release();
        btnStartNormalTex_ = nullptr;
    }
    if (btnStartHoverTex_)
    {
        btnStartHoverTex_->Release();
        btnStartHoverTex_ = nullptr;
    }
    if (btnStartPressTex_)
    {
        btnStartPressTex_->Release();
        btnStartPressTex_ = nullptr;
    }
    if (btnStartDisabledTex_)
    {
        btnStartDisabledTex_->Release();
        btnStartDisabledTex_ = nullptr;
    }
}

bool LauncherView::RenderEpicGameStartButton(
    const char* id,
    const ImVec2& size,
    bool isLocked) noexcept
{
    ImGui::PushID(id);
    bool pressedResult = ImGui::InvisibleButton("##epic_start", size);
    bool hovered = ImGui::IsItemHovered();
    bool held = ImGui::IsItemActive();
    ImVec2 p0 = ImGui::GetItemRectMin();
    ImVec2 p1 = ImGui::GetItemRectMax();
    ...
    const float t = static_cast<float>(ImGui::GetTime());
    const float yOffset = (held && !isLocked) ? 1.5f : 0.0f;
    ...
    if (!btnStartNormalTex_)
    {
        ImGui::PopID();
        if (fontLarge_) ImGui::PushFont(fontLarge_);
        bool res = ModernButton(id, lang::GetString("launcher_game_start").c_str(), size, isLocked, ButtonIcon::Play, IM_COL32(37, 99, 235, 255), !isLocked);
        if (fontLarge_) ImGui::PopFont();
        return res;
    }

    // 1. Ambient / Radiant sapphire glow behind the button
    if (!isLocked)
    {
        const float pulse = (sinf(t * 3.2f) * 0.5f + 0.5f);
        const float glowAlpha = (30.0f + 55.0f * hoverVal + 20.0f * pulse);
        const float glowExpand = 6.0f + 4.0f * hoverVal;
        
        dl->AddRectFilled(
            ImVec2(drawP0.x - glowExpand, drawP0.y - glowExpand + 2.0f),
            ImVec2(drawP1.x + glowExpand, drawP1.y + glowExpand + 2.0f),
            IM_COL32(14, 165, 233, static_cast<int>(glowAlpha)),
            16.0f);
    }

    // 2. Render appropriate texture state
    if (isLocked)
    {
        IDirect3DTexture9* tex = btnStartDisabledTex_ ? btnStartDisabledTex_ : btnStartNormalTex_;
        dl->AddImage(
    ...
    else if (held && btnStartPressTex_)
    {
        // Pressed state: slight scale down / press sensation
        ImVec2 pressP0 = ImVec2(drawP0.x + 1.0f, drawP0.y + 1.0f);
        ImVec2 pressP1 = ImVec2(drawP1.x - 1.0f, drawP1.y - 1.0f);

        dl->AddImage(
            reinterpret_cast<ImTextureID>(btnStartPressTex_),
            pressP0,
            pressP1,
            btnStartUvMin_,
            btnStartUvMax_,
    ...
    else
    {
        // Normal state base
        dl->AddImage(
            reinterpret_cast<ImTextureID>(btnStartNormalTex_),
            drawP0,
            drawP1,
            btnStartUvMin_,
            btnStartUvMax_,
        ...
        // Hover overlay blended on top with alpha based on hoverVal
        if (btnStartHoverTex_ && hoverVal > 0.005f)
        {
            const int hoverAlpha = static_cast<int>(hoverVal * 255.0f);
            dl->AddImage(
                reinterpret_cast<ImTextureID>(btnStartHoverTex_),
                drawP0,
                drawP1,
                btnStartUvMin_,
                btnStartUvMax_,
                IM_COL32(255, 255, 255, (std::min)(255, hoverAlpha)));
        }

        // 3. Subtle magical sapphire floating sparkles around corners when hovered
        if (hoverVal > 0.1f)
        {
            for (int i = 0; i < 4; ++i)
            {
                float seed = static_cast<float>(i) * 1.57f;
                float spkTime = t * 2.5f + seed;
                float spkAlpha = (sinf(spkTime * 3.0f) * 0.5f + 0.5f) * hoverVal * 200.0f;
                float spkX = (i % 2 == 0) ? drawP0.x + 8.0f + sinf(spkTime) * 6.0f : drawP1.x - 8.0f - sinf(spkTime) * 6.0f;
                float spkY = (i < 2) ? drawP0.y + 8.0f + cosf(spkTime) * 6.0f : drawP1.y - 8.0f - cosf(spkTime) * 6.0f;
                
                dl->AddCircleFilled(ImVec2(spkX, spkY), 1.8f, IM_COL32(56, 189, 248, static_cast<int>(spkAlpha)));
                dl->AddCircleFilled(ImVec2(spkX, spkY), 0.9f, IM_COL32(255, 255, 255, static_cast<int>(spkAlpha)));
            }
        }
    }
